...
The primary change introduced in this branch was to `versions.json`, adding `jammy` variant. The other changes are generated by executing the following command, which uses Docker so it works on any platform.
...
After running the command above the Dockerfiles for the target architecture of the host/docker machine are created or updated. The commands below build the images locally, using `dockerx` multi platform build.
